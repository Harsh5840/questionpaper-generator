...
function toBackendRequest(request: PaperRequest) {
  return {
    ...
    section_blueprint: request.sectionBlueprint?.map((section) => ({
      id: section.id,
      title: section.title,
...
function normalizeRetrievalResults(value: unknown): RetrievalResult[] {
  ...
  return value.map((item) => {
    ...
    return {
      ...
      citations: Array.isArray(record.citations) ? record.citations.map(String) : undefined,
      ...
      chapter: record.chapter || metadata.chapter ? String(record.chapter ?? metadata.chapter) : undefined,
      topic: metadata.topic ? String(metadata.topic) : undefined,
      sectionLabel: record.section_label || metadata.section_label || signals.section_label ? String(record.section_label ?? metadata.section_label ?? signals.section_label) : undefined,
      sectionType: metadata.section_type ? String(metadata.section_type) : undefined,
      bookId: record.book_id || metadata.book_id ? String(record.book_id ?? metadata.book_id) : undefined,
      bookTitle: record.book_title || metadata.book_title ? String(record.book_title ?? metadata.book_title) : undefined,
      publisher: record.publisher || metadata.publisher ? String(record.publisher ?? metadata.publisher) : undefined,
      bookType: record.book_type || metadata.book_type ? String(record.book_type ?? metadata.book_type) : undefined,
      chapterId: record.chapter_id || metadata.chapter_id ? String(record.chapter_id ?? metadata.chapter_id) : undefined,
      sectionTitle: record.section_title || metadata.section_title ? String(record.section_title ?? metadata.section_title) : undefined,
      page: record.page || metadata.page ? Number(record.page ?? metadata.page) : undefined,
      questionId: record.question_id || metadata.question_id ? String(record.question_id ?? metadata.question_id) : undefined,
      category: record.category || metadata.category ? String(record.category ?? metadata.category) : undefined,
      sourceLabel: record.source_label || metadata.source_label ? String(record.source_label ?? metadata.source_label) : undefined,
      orderIndex: record.order_index || metadata.order_index ? Number(record.order_index ?? metadata.order_index) : undefined,
      skills: Array.isArray(record.skills) ? record.skills.map(String) : undefined,
      formulas: Array.isArray(record.formulas) ? record.formulas.map(String) : undefined,
    };
  });
}
